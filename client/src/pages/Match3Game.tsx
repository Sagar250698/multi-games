          ...
          {board.map((row, y) =>
            row.map((tile, x) => (
              <motion.div
                ...
                className={`w-10 h-10 md:w-14 md:h-14 rounded-xl cursor-pointer shadow-inner relative transition-colors duration-300 ${selectedTile?.x === x && selectedTile?.y === y ? 'ring-4 ring-white ring-inset scale-110 z-10' : ''
                  }`}
              >
                {tile.color && (
